...
| G-9 | **Next.js bundles picomatch 4.0.3 with a known ReDoS** (`GHSA-c2c7-rcm5-vvqj`). `next/dist/compiled/picomatch/index.js` is a pre-compiled bundle that Next.js uses internally for glob matching. As of `next@16.2.3` (latest on 2026-04-08) the bundled file is byte-identical to `16.1.7`. | HIGH (impact) / LOW (exploitability) | **Accepted**, tracked in `.grype.yaml`. Non-exploitable — the bundled picomatch is not user-reachable. Only Next.js internals invoke it (build-time glob resolution, `next.config.ts` parsing, static asset matching). HLEcosystem code does not pass attacker-controlled patterns to picomatch. Review by 2026-05-08. |
| G-10 | **Node 22.22.2 LTS has 7 unpatched CVEs** (CVE-2026-21710/13/14/15/16/17 + one medium V8 issue). Node maintainers have not yet cut 22.22.3 — the 22.x branch fix is still pending upstream as of 2026-04-09. | MIXED | **Accepted**, tracked in `.grype.yaml`. Per-CVE reachability analysis: CVE-2026-21710 requires `req.headersDistinct`, not used by HLEcosystem code; CVE-2026-21713 is an HMAC timing issue, not used (sessions are opaque DB tokens); CVE-2026-21714 requires HTTP/2, not used; CVE-2026-21715/16 require Node's `--permission` flag, not used; CVE-2026-21717 (V8 DoS via JSON.parse hash collisions) has partial mitigation via zod input length caps. Dependabot's docker ecosystem will auto-PR the base image digest when `node:22-alpine` publishes a rebuild. Review by 2026-05-08. |
| G-11 | **OpenSSL 3.5.5 TLS 1.3 key exchange defect** (CVE-2026-2673) in `libcrypto3` / `libssl3` from alpine 3.23. Alpine is waiting for OpenSSL 3.5.6 to release. | HIGH (impact) / N/A (reachability) | **Accepted**, tracked in `.grype.yaml`. HLEcosystem Next.js apps do NOT terminate TLS — the Next process runs plain HTTP on a Podman internal network behind an operator-provided reverse proxy (nginx, Cloudflare Tunnel, etc.) that performs TLS termination. The vulnerable OpenSSL code path is server-side TLS 1.3 key exchange group selection, which is never executed by HLEcosystem because HLEcosystem never does the TLS server handshake. Review by 2026-05-08. |
