// Pure amortization math (ported verbatim from legacy lib/amortization.ts).
// Client-importable: the debt detail page runs the extra-payment calculator
// in the browser.
...
export function calculateAmortization(
  ...
  while (remaining > 0.01 && month < maxMonths) {
    ...
    const totalPaymentThisMonth = Math.min(
      remaining + interest,
...
export function calculateExtraPaymentSavings(
  ...
  const withoutExtra = calculateAmortization(
    balance,
    annualRate,
    monthlyPayment
  )
