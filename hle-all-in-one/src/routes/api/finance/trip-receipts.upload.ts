...
import { FINANCE_UPLOAD_EXTENSIONS } from "@/server/finance/taxes"
import { attachTripExpenseReceipt } from "@/server/finance/trips"

const UUID_RE =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i

export const Route = createFileRoute("/api/finance/trip-receipts/upload")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const result = await attachTripExpenseReceipt(
          auth.householdId,
          expenseId,
          { buffer, originalName: validation.sanitizedName }
        )
        if ("error" in result) {
