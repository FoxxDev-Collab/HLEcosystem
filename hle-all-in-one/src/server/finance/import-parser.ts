// Bank statement parsers, ported VERBATIM (same semantics) from
// hle-family_finance/lib/import-parser.ts. Pure functions — no DB, no IO.
// Covered by import-parser.test.ts; do not change parsing behavior without
...
function detectWellsFargoColumns(
  ...
  if (firstFields[0]?.trim().toLowerCase() === "date") {
    ...
    const findIdx = (...names: Array<string>) =>
      header.findIndex((h) => names.includes(h))
    const di = findIdx("date")
    const desci = header.findIndex(
      (h) => h.includes("description") || h.includes("memo")
    )
  ...
  const legacyAmount = Number.parseFloat(
    firstFields[1]?.replaceAll(/[,"$]/g, "") ?? ""
  )
...
function parseWellsFargoLine(
  line: string,
  cols: WellsFargoColumns
  ...
  const amount = Number.parseFloat(
    (fields[cols.amountIdx] ?? "").replaceAll(/[,"$]/g, "")
  )
 ...
 */
export function parseWellsFargoCSV(content: string): Array<ParsedTransaction> {
  const lines = content.trim().split(/\r?\n/)
  const firstLine = lines.find((l) => l.trim()) ?? ""
  const cols = detectWellsFargoColumns(parseCSVLine(firstLine))

  const transactions: Array<ParsedTransaction> = []
 ...
 */
export function parseGenericCSV(content: string): Array<ParsedTransaction> {
  ...
  const dateIdx = header.findIndex(
    (h) => h.includes("date") || h === "posted date" || h === "transaction date"
  )
  ...
  const descIdx = header.findIndex(
    (h) =>
      h.includes("description") || h.includes("memo") || h.includes("narrative")
  )
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (amountIdx !== -1) {
      ...
      const debit =
        parseFloat(fields[debitIdx]?.replace(/[,"$]/g, "") || "0") || 0
      const credit =
        parseFloat(fields[creditIdx]?.replace(/[,"$]/g, "") || "0") || 0
 ...
 */
export function parseOFX(content: string): Array<ParsedTransaction> {
  const transactions: Array<ParsedTransaction> = []
...
function extractPayee(description: string): string {
  ...
  let payee = description
    .replace(/\s+/g, " ")
    .replace(
      /\b(POS|DEBIT|CREDIT|ACH|WIRE|CHECK|ATM|WITHDRAWAL|DEPOSIT)\b/gi,
      ""
    )
