// Thin TMDB client, ported from hle-media/src/server/tmdb.ts. Optional — if
// TMDB_API_KEY is unset, every call returns null and the scanner is
// unaffected.
//
// Uses TMDB v3 with the api_key query param. Rate limit: TMDB allows
// ~50 req/s globally; the enrichment loop is sequential per-item so we never
// approach that ceiling on a personal library.
