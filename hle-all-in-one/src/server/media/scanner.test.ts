// Ported from hle-media/src/server/scanner.test.ts (bun:test → vitest).
// Pure-function tests for the filename parser — no DB needed.
import { describe, expect, test } from "vitest"
import { parsePath } from "./scanner-parse"

describe("parsePath", () => {
  test("movie at top level with year folder", () => {
    expect(parsePath("Movies/Inception (2010)/Inception (2010).mkv")).toEqual({
      kind: "movie",
      title: "Inception",
      year: 2010,
    })
  ...
  test("episode with no episode title", () => {
    expect(parsePath("Shows/Foo/Season 02/Foo S02E10.mp4")).toEqual({
      kind: "episode",
      seriesTitle: "Foo",
      seriesYear: null,
      season: 2,
  ...
  test("season number from filename when no Season folder present", () => {
    ...
    expect(parsePath("Shows/My Show/My Show - S05E12 - Finale.mp4")).toEqual({
      kind: "episode",
      seriesTitle: "My Show",
      seriesYear: null,
      season: 5,
