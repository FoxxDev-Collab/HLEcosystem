// Filename → movie/episode parsing, ported from
// hle-media/src/server/scanner-parse.ts. Pure functions — covered by
// scanner.test.ts.
...
export function parsePath(relPath: string): Parsed {
  ...
  // Series detection — some segment matches "Season N", or the top-level folder
  ...
  // Movie: prefer the parent folder name (e.g. "Inception (2010)/Inception
  // (2010).mkv"); fall back to the bare filename. Skip the parent if it is
  // itself a category folder ("Movies/").
