// Scan-run tracking, ported from hle-media/src/server/scan-runs.ts.
//
// In-memory only. Lost on restart, which is fine — scans are user-triggered
// and can be re-run trivially. Keeps state simple; no schema needed (legacy
// had no table for this either — see hle-media/migrations/0001_init.sql).
import { randomUUID } from "node:crypto"
import { scanLibrary } from "./scanner"
import { enrichHousehold } from "./enrichment"
import type { ScanSummary } from "./scanner"
...
export function startScan(opts: {
  ...
  // re-trigger enrichment manually.
  void (async () => {
    try {
